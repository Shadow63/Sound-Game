...
export function renderImage() {
    ...
    let hold = `
    <div id="picture">
        <img width="500" height="500" src="assests/Pictures/questionMark.jpg" alt="Question Mark" id="image"></img> 
    </div>  
    `;
...
$.getJSON("sounds.json", function(json) {
    ...
    for (let i = 0; i < testArray.length; i++) {
        testArray[i].audio = new Audio('assets/Sounds/' + testArray[i].name + '.mp3');
        testArray[i].image = 'assets/Pictures/' + testArray[i].name + '.jpeg';
    }
});
